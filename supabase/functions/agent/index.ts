...
async function searchKnowledge(sb: ReturnType<typeof getSupabase>, query: string): Promise<KnowledgeResult> {
  ...
  if (embedding) {
    ...
    if (!error && data && data.length > 0) {
      const chunks: ChunkResult[] = (data as Record<string, unknown>[]).map((r) => ({
...
// ─── Orchestration ───
const ORCHESTRATOR_PROMPT = `You are an AI orchestrator for OnboardAI at NovaTech. Analyze the user query and return JSON specifying which tools to use. Return only valid JSON with no markdown fences.

Available tools:
- searchKnowledge: Search org documents (policies, guides, handbooks). Use for ANY question about company information, policies, or procedures.
- getOnboardingTasks: Get the user's onboarding task list and progress. Use when user asks about their tasks, what to do next, or their progress.
- getUserProfile: Get the user's name, role, department. Use for personalized responses.
- findContact: Find a contact person. Use when user asks who to talk to, or needs a specific department contact.
- createAccessRequest: Create an access request. ONLY use if the user EXPLICITLY says "request access", "get me access", "create an access request", or "give me access". Do NOT use just because the user asks about policies.
- updateTaskStatus: Update a task status. Use only when explicitly confirming task completion.

Return this exact JSON structure:
{"intent":"brief description","tools":["tool1"],"searchQuery":"query if searchKnowledge used","contactQuery":"issue if findContact used","accessResource":"resource if createAccessRequest used","taskTitle":"title if updateTaskStatus used","taskStatus":"status if updateTaskStatus used"}
...
"Request GitHub access for me" -> {"intent":"access request","tools":["searchKnowledge","getUserProfile","findContact","createAccessRequest"],"searchQuery":"github access policy requirements approval","contactQuery":"github team lead approval","accessResource":"GitHub"}
"I just joined the ML team, help me get set up" -> {"intent":"multi-step onboarding","tools":["getUserProfile","getOnboardingTasks","searchKnowledge","findContact"],"searchQuery":"ML team onboarding guide getting started","contactQuery":"team lead ml onboarding"}
...
async function orchestrate(query: string): Promise<Plan> {
  ...
  if (response) {
    try { return JSON.parse(response) as Plan; } catch { /* fall through to keyword fallback */ }
  }

  // Keyword fallback when no LLM API key or LLM fails
  const q = query.toLowerCase();
  if ((q.includes("request") || q.includes("get me") || q.includes("give me")) &&
  ...
  if (q.includes("just joined") || q.includes("help me get everything") || q.includes("get everything set up") || q.includes("get started")) {
  ...
  if (q.includes("what") && (q.includes("left") || q.includes("remaining") || q.includes("still need") || q.includes("todo") || q.includes("progress"))) {
    return { intent: "onboarding_status", tools: ["getUserProfile", "getOnboardingTasks"] };
  }
  if (q.includes("who") || (q.includes("contact") && !q.includes("access")) || q.includes("laptop") || q.includes("hardware") || q.includes("vpn") || q.includes("password")) {
...
async function generateResponse(
  ...
  if (tasks && tasks.length > 0) {
    ...
    ctxParts.push(`ONBOARDING TASKS:\nCompleted: ${done || "none"}\nPending/In-Progress: ${pending || "none"}`);
  }
  ...
  const req = toolData.createAccessRequest as { requestId: string; approver: string; approverRole: string } | null | undefined;
  if (req) ctxParts.push(`ACCESS REQUEST CREATED: ID ${req.requestId}, Approver: ${req.approver} (${req.approverRole}), Status: Pending`);
  ...
  const knowledgeInstructions = hasKnowledge || !isKnowledgeQuery
    ...
    : "The knowledge base does not contain enough information. Clearly state that you could not find the information, then offer to connect the user with an appropriate contact.";

  const systemPrompt = `You are OnboardAI, the employee onboarding assistant for NovaTech. Be concise and friendly.\n${knowledgeInstructions}\nIMPORTANT: Never fabricate policies, contacts, or procedures not in the context.\n\nContext:\n${ctxParts.join("\n\n")}`;
...
function buildDemoResponse(plan: Plan, toolData: Record<string, unknown>): { greeting: string; summary: string } {
  ...
  if (kr) {
    if (kr.chunks.length > 0) {
      parts.push(`Based on the NovaTech knowledge base:\n\n${kr.chunks.slice(0, 3).map((c) => `[${c.document_title} — ${c.section_label}]:\n${c.content.slice(0, 500)}`).join("\n\n")}`);
  ...
  const contact = toolData.findContact as Record<string, unknown> | undefined;
  if (contact) {
    parts.push(`For that issue, contact:\n\n${contact.name} — ${contact.role}\nDepartment: ${contact.department}\nEmail: ${contact.email}\nPhone: ${contact.phone}`);
  } else if (plan.tools.includes("findContact") && !contact) {
    parts.push("I could not find a specific contact for this issue. Please reach out to your team lead or HR.");
  }

  const req = toolData.createAccessRequest as { requestId: string; approver: string; approverRole: string; status: string } | null | undefined;
  if (req !== undefined) {
    if (req) {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    let accessRequest: { requestId: string; approver: string; approverRole: string; status: string } | undefined;
    let taskUpdated = false;

    for (const tool of plan.tools) {
      if (tool === "searchKnowledge") {
        const usedVector = mode === "ai" && llmApiKey;
        activity.push({
          icon: "Search", title: "Searching organizational knowledge",
          description: usedVector ? "Semantic vector similarity search" : "Keyword search (demo mode)",
      ...
      if (tool === "createAccessRequest") {
        activity.push({ icon: "FileCheck2", title: "Creating access request", description: `Submitting ${plan.accessResource || "resource"} access request`, status: "completed", tool: "RequestCreator", detail: "Verifying requirements and creating request", event_type: "STATE_UPDATED" });
        const req = await createAccessRequest(sb, plan.accessResource || "GitHub", uid);
        toolData.createAccessRequest = req;
        accessRequest = req || undefined;
        if (req) {
          await updateTaskStatus(sb, "GitHub", uid, "in-progress");
          taskUpdated = true;
          activity[activity.length - 1].detail = `Request ${req.requestId} sent to ${req.approver}`;
          await sb.from("agent_activity").insert({ user_id: uid, icon: "FileCheck2", title: `Created access request ${req.requestId}`, description: `${plan.accessResource || "GitHub"} access pending approval from ${req.approver}`, timestamp: "Just now", status: "completed", tool: "RequestCreator", detail: `Request ID: ${req.requestId}`, event_type: "STATE_UPDATED" });
        }
      }
    ...
    const isKnowledgeOnlyQuery = plan.tools.includes("searchKnowledge") &&
      !plan.tools.includes("getOnboardingTasks") && !plan.tools.includes("createAccessRequest");
    ...
    if (noKnowledgeFound) {
      ...
      if (escalationContact) {
        finalSummary += `\n\n${escalationContact.name} (${escalationContact.role})\nEmail: ${escalationContact.email}\nPhone: ${escalationContact.phone}`;
      } else {
    ...
    // Log all activity events to DB
    for (const event of activity) {
      await sb.from("agent_activity")
        .insert({ user_id: uid, ...event, timestamp: "Just now" })
